...
"""
@quickHyper: input =>[a, b, c, d]
h1 = hypernym of wn.synsets(a)[0] and wn.synsets(b)[0]
h2 = hypernym of wn.synsets(h1)[0] and wn.synsets(c)[0]
hn = hypernym of wn.synsets(hn-1)[0] and wn.synsets([-1])[0]
test: print quickHyper(["lobe", "brain", "gyrus"])
"""
def quickHyper(itemArr, debug=False):
	firstH = wn.synsets(itemArr[0])[0].lowest_common_hypernyms(wn.synsets(itemArr[1])[0])
	if firstH == []:
		return "Root"
	#print firstH	
	for k in itemArr[2:]:
		try:
			H = firstH[0].lowest_common_hypernyms(wn.synsets(k)[0])
			if H != []:
				firstH = H
		except:
			return "Root"
	if firstH[0].name.split(".")[0] in itemArr:
		firstH = firstH[0].hypernyms()
	return firstH[0].name.split(".")[0]
